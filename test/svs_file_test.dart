...
void main() {
  ...
  test(
    'classifies pyramid levels and associated images, parses metadata',
    () async {
      ...
      expect(
        svs.associatedImages[1].isDecodable,
        isFalse,
      ); // compression=1, not JPEG
      expect(
        svs.associatedImages[2].kind,
  ...
  test(
    'readStripJpegBytes on a non-decodable associated image throws clearly',
    () async {
      final bytes = buildTiff(
        bigTiff: false,
        order: Endian.little,
    ...
    final bytes = buildTiff(
      ...
      ifds: [
        tiledLevelTags(
          ...
          height: 256,
          tileSize: 256,
          compression: ApCompression.jp2k,
        ),
      ],
    );
    final path = await writeBytes(bytes);
  ...
  test(
    'rejects a level whose tile table length does not match its tile grid',
    () async {
