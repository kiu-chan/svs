...
real Aperio pyramid takes. Like `exportSvsLevel`, a `maxPixels` safety guard
(default matches `exportSvsLevel`'s) throws before attempting to build a
pyramid from an accidentally huge source rectangle — the whole crop is
decoded and pyramid-built in memory.
